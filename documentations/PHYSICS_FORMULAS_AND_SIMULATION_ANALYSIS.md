...
- `acc_start_si` : Starting acceleration capability (m/s²)
...
- `acc_start_si` is in **m/s²** (input parameter)
...
```cpp
f_start = mass_totalInertial × acc_start_si + f_resStart
```
...
1. Initialize:
   ...
   - Set initial conditions: v = 0, acc = acc_start_si, position = 0
...
```
p_wheel = [(mass × acc_start_si × v_p1) / v] × (v / cV)
        = (mass × acc_start_si × v_p1) / cV
...
// Need: acc_start_si for simulation
...
// P = f_start × v_p1
// f_start = mass_totalInertial × acc_start_si (ignoring resistance)
// Therefore: P = mass × acc_start_si × v_p1

// Solving for acc_start_si:
acc_start_si = P_total_kW / (mass_totalInertial × v_p1)

// In code:
double v_p1_ms = movingData->v_p1 / 3.6;  // km/h → m/s
double mass_kg = massData->mass_totalInertial × 1000;  // tons → kg
movingData->acc_start_si = (P_total_kW × 1000) / (mass_kg × v_p1_ms);  // m/s²
```
...
#### Stage 2: Physics Simulation (Accepts acc_start_si)

**Convert Power → Acceleration:**

```cpp
double acc_start_si_required = (P_total_kW × 1000) / (mass_kg × v_weak_ms);
```

**Set in simulation:**

```cpp
movingData->acc_start_si = acc_start_si_required;  // m/s²
```
...
for (double P_test = P_min; P_test <= P_max; P_test += P_step) {
    ...
    // 2. Set in simulation
    movingData->acc_start_si = acc_test;
...
movingData->acc_start_si = optimalAccel;
runDynamicSimulation();
...
4. **Optimization should sweep `acc_start_si`, not power directly**
   - Calculate baseline power using thesis formula
   - Convert to `acc_start_si` for simulation
