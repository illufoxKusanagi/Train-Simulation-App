...
//   qDebug() << "=========================================================";
//   qDebug() << "AppContext Data Snapshot at
//   TrainSimulationHandler::initData()"; qDebug() <<
//   "=========================================================";

//   // --- Data from TrainParameterPage (via TrainDataHandler) ---
//   qDebug() << "\n--- TrainParameterPage Data (TrainDataHandler) ---";
//   if (trainData) {
//     qDebug() << "TrainData:";
//     qDebug() << "  Number of Cars (n_car):" << trainData->n_car;
//     qDebug() << "  Number of Tc (n_Tc):" << trainData->n_Tc;
//     qDebug() << "  Number of M1 (n_M1):" << trainData->n_M1;
//     qDebug() << "  Number of M2 (n_M2):" << trainData->n_M2;
//     qDebug() << "  Number of T1 (n_T1):" << trainData->n_T1;
//     qDebug() << "  Number of T2 (n_T2):" << trainData->n_T2;
//     qDebug() << "  Number of T3 (n_T3):" << trainData->n_T3;
//     qDebug() << "  M1 Disabled (n_M1_disabled):" << trainData->n_M1_disabled;
//     qDebug() << "  M2 Disabled (n_M2_disabled):" << trainData->n_M2_disabled;
//     qDebug() << "  Traction Motors (n_tm):" << trainData->n_tm;
//     qDebug() << "  Axles (n_axle):" << trainData->n_axle;
//     qDebug() << "  Wheel Diameter (wheel):" << trainData->wheel;
//     qDebug() << "  Gear Ratio (gearRatio):" << trainData->gearRatio;
//     qDebug() << "  Trainset Length (trainsetLength):"
//              << trainData->trainsetLength;
//   } else {
//     qDebug() << "TrainData is NULL!";
//   }

//   if (massData) {
//     qDebug() << "MassData (individual car masses):";
//     qDebug() << "  Mass Tc (mass_TC):" << massData->mass_TC;
//     qDebug() << "  Mass M1 (mass_M1):" << massData->mass_M1;
//     qDebug() << "  Mass M2 (mass_M2):" << massData->mass_M2;
//     qDebug() << "  Mass T1 (mass_T1):" << massData->mass_T1;
//     qDebug() << "  Mass T2 (mass_T2):" << massData->mass_T2;
//     qDebug() << "  Mass T3 (mass_T3):" << massData->mass_T3;
//     qDebug() << "  Inertial Coeff Motor (i_M):" << massData->i_M;
//     qDebug() << "  Inertial Coeff Trailer (i_T):" << massData->i_T;
//   } else {
//     qDebug() << "MassData is NULL!";
//   }

//   if (loadData) {
//     qDebug() << "LoadData:";
//     qDebug() << "  Load per Car (load):" << loadData->load;
//     qDebug() << "  Passenger Weight (mass_P):" << loadData->mass_P;
//     qDebug() << "  Passengers Tc (n_PTc):" << loadData->n_PTc;
//     qDebug() << "  Passengers M1 (n_PM1):" << loadData->n_PM1;
//     qDebug() << "  Passengers M2 (n_PM2):" << loadData->n_PM2;
//     qDebug() << "  Passengers T1 (n_PT1):" << loadData->n_PT1;
//     qDebug() << "  Passengers T2 (n_PT2):" << loadData->n_PT2;
//     qDebug() << "  Passengers T3 (n_PT3):" << loadData->n_PT3;
//     // Add AW condition if it's stored, e.g., loadData->aw_condition_index
//   } else {
//     qDebug() << "LoadData is NULL!";
//   }

//   // --- Data from ElectricalParameterPage (via ElectricalDataHandler) ---
//   qDebug() << "\n--- ElectricalParameterPage Data (ElectricalDataHandler)
//   ---"; if (efficiencyData) {
//     qDebug() << "EfficiencyData (Static Values):";
//     qDebug() << "  Gearbox Eff (stat_eff_gear):"
//              << efficiencyData->stat_eff_gear;
//     qDebug() << "  VVVF Eff (stat_eff_vvvf):" <<
//     efficiencyData->stat_eff_vvvf; qDebug() << "  Motor Eff
//     (stat_eff_motor):"
//              << efficiencyData->stat_eff_motor;
//     // For CSV data, you might want to print size or first few elements if
//     // needed
//     qDebug() << "  Gearbox Eff CSV size:" << efficiencyData->eff_gear.size();
//     qDebug() << "  VVVF Eff CSV size:" << efficiencyData->eff_vvvf.size();
//     qDebug() << "  Motor Eff CSV size:" << efficiencyData->eff_motor.size();
//   } else {
//     qDebug() << "EfficiencyData is NULL!";
//   }

//   // Original mass calculations
//   massData->mass_totalEmpty = m_massHandler->countMassEmptyCar();
//   if (loadData->load > 0) {
//     massData->mass_totalLoad = m_massHandler->countMassLoadInput();
//     massData->mass_totalInertial = m_massHandler->countInertialMassInput();
...
//   qDebug()
//       << "\n--- Calculated Masses (after massHandler calls in initData) ---";
//   if (massData) {
//     qDebug() << "  Total Empty Mass:" << massData->mass_totalEmpty;
//     qDebug() << "  Total Load Mass (Operational):" <<
//     massData->mass_totalLoad; qDebug() << "  Total Inertial Mass:" <<
//     massData->mass_totalInertial;
//   } else {
//     qDebug() << "MassData is NULL (cannot print calculated masses)!";
//   }
//   qDebug() <<
//   "--------------------------------------------------------------"; qDebug()
//   << "================ End of AppContext Snapshot ================\n";

//   movingData->v = 0.0; // Initial speed
//   // acc_start and decc_start should have been set by RunningDataHandler if
//   UI
//   // provides them
//   movingData->acc = movingData->acc_start;
//   movingData->decc =
//       movingData->decc_start *
//       constantData->cV; // This might need decc_start to be non-zero
// }
