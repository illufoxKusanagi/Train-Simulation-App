...
void TrainSimulation::calculateBrakingForce() {
  ...
  } else if (movingData->v > movingData->v_b1 * movingData->v <=
             movingData->v_b2) {
    resistanceData->f_motor =
...
//   int coastingCount = 0;
//   movingData->v = 0.0;
//   qDebug() << "Acceleration at start is : " <<
//   movingData->acc; while (movingData->v >= 0) {
//     resistanceData->f_resStart = calculateStartRes();
//     resistanceData->f_resRunning =
//     calculateRunningRes(movingData->v); if (isAccelerating) {
...
//       }
//       phase = "Accelerating";
//       calculatePoweringForce(movingData->acc,
//       movingData->v);
...
//       movingData->v += movingData->acc *
//       constantData->dt;
//     } else if (isCoasting) {
//       if (movingData->v <= (movingData->v_limit -
//       movingData->v_diffCoast)) {
//         isCoasting = false;
//         isAccelerating = true;
//         coastingCount++;
//         if (coastingCount >= 3) {
...
//           resistanceData->f_total /
//           massData->m_totalInertial;
//       movingData->v += movingData->decc *
//       constantData->dt; if (movingData->v <= 0)
//         break;
//     }
//     trainMotorData->tm_f_res =
//     calculateResistanceForcePerMotor(
//         movingData->v > 0 ?
//         resistanceData->f_resRunning
//                          : resistanceData->f_resStart);
...
void TrainSimulation::simulateDynamicTrainMovement() {
  ...
  // Initial debug of key parameters
