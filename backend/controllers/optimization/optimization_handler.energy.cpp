...
//   ───────────────── auto travelTime =
//   std::make_shared<FuzzyVariable>("TravelTime", minT, maxT);
...
//   ───────────────────────────────────────────── auto timeScore =
//   std::make_shared<FuzzyVariable>("TimeScore", 0.0, 100.0);
//   timeScore->addTerm(
...
//   ──────────── auto motorPower =
//   std::make_shared<FuzzyVariable>("MotorPower", minP, maxP);
...
//   ──────────────────────────────────────────── auto powerScore =
//   std::make_shared<FuzzyVariable>("PowerScore", 0.0, 100.0);
//   powerScore->addTerm(
//       std::make_shared<TrapezoidSet>("Poor", 0.0, 0.0, 15.0, 30.0));
...
// //   ──────────────────────────────────────────── auto energyScore =
// //   std::make_shared<FuzzyVariable>("EnergyScore", 0.0, 100.0);
// //   energyScore->addTerm(
// //       std::make_shared<TrapezoidSet>("Poor", 0.0, 0.0, 15.0, 30.0));
// //
...
// //   energyScore->addTerm(
// //       std::make_shared<TrapezoidSet>("Excellent", 68.0, 82.0, 100.0,
// 100.0));
...
// //   auto makeRule = [](const QString &term, const QString &score) ->
// FuzzyRule
// //   {
// //     FuzzyRule r;
...
// // This gives equal weight to travel time, motor power, and energy
// consumption. double OptimizationHandler::evaluateFuzzyScore(double
// travelTime,
//                                                double motorPower)
...
// //   Pass 1: run all 20 simulations, collect (travelTime, peakPower) per
// combo
...
//       // runDynamicSimulation() acquires m_simulationMutex internally for
//       each
//       // step — holding it here too would deadlock (QMutex is non-recursive).
//       m_movingData->acc_start_si = acc;
//       m_movingData->v_p1 = vp1;

//       // Block signals so simulationCompleted is NOT emitted during the
//       sweep.
...
//       // different scores on repeated runs. initData() +
//       clearSimulationDatas()
//       // inside runDynamicSimulation() already reset all state we need; we
//       don't
...
//   // double minE = rawData[0].energyConsumption,
//   //        maxE = rawData[0].energyConsumption;
