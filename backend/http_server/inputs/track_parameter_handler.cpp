...
QHttpServerResponse TrackParameterHandler::handleUpdateTrackParameters(const QJsonObject &data)
{
    ...
    try
    {
        QJsonObject trackParams = data.contains("trackParameters") ? data["trackParameters"].toObject() : data;
        ...
        {
            m_context.stationData->stat_x_station = trackParams["x_station"].toDouble();
        }
        ...
        {
            m_context.stationData->stat_dwellTime = trackParams["dwellTime"].toDouble();
        }

        // Handle CSV array data for vectors
        if (trackParams.contains("x_station_array") && trackParams["x_station_array"].isArray())
        {
        ...
        if (trackParams.contains("v_limit_array") && trackParams["v_limit_array"].isArray())
        {
        ...
        if (trackParams.contains("slope_array") && trackParams["slope_array"].isArray())
        {
        ...
        if (trackParams.contains("radius_array") && trackParams["radius_array"].isArray())
        {
        ...
        if (trackParams.contains("dwellTime_array") && trackParams["dwellTime_array"].isArray())
        {
