...
QHttpServerResponse ApiHandler::handleQuickInit()
{
    ...
    try
    {
        ...
        if (m_context.trainData)
        {
            ...
            qDebug() << "✓ Train parameters initialized (matching backend copy: 12-car configuration)";
        }
        ...
        if (m_context.massData)
        {
            ...
            m_context.massData->mass_totalEmpty = 0.0; // Will be calculated by countMassEmptyCar()
            m_context.massData->mass_totalLoad = 0.0;
            m_context.massData->mass_totalInertial = 0.0;
            qDebug() << "✓ Mass parameters initialized (matching backend copy defaults)";
        }
        ...
        if (m_context.loadData)
        {
            ...
            qDebug() << "✓ Load parameters initialized (matching backend copy defaults)";
        }

        // Motor data (realistic values for 8-car EMU with 24 motors)
        if (m_context.trainMotorData)
        {
            m_context.trainMotorData->tm_f = 250.0;  // Total traction force (kN) - much higher for realistic acceleration
        ...
        {
            m_context.resistanceData->startRes = 30.0; // Starting resistance (reduced for better acceleration)
        ...
        if (m_context.stationData)
        {
            ...
            m_context.stationData->stat_radius = 0.0;
        ...
        response["status"] = "success";
        response["message"] = "Quick initialization completed with minimal valid values";
        response["initialized"] = QJsonObject{
            {"trainData", m_context.trainData != nullptr},
            {"massData", m_context.massData != nullptr},
            {"loadData", m_context.loadData != nullptr},
        ...
        qDebug() << "  mass_totalInertial:" << m_context.massData->mass_totalInertial;
